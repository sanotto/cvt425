*SRCMBRTXT:OJ-Import Arch. Internet-Filtr Cltes CO
      *//OVRDBF FILE(RECMBR) TOFILE(OJMOFI)
      *//OVRDBF FILE(UPDMBR) TOFILE(OJMOFI)
      *//CRTBNDRPG PGM(DESA00525/OJIM10R2) SRCFILE(DESA00525/QRPGSRC)

      * Empresas dañinas @RGENET SRL
     ...
     D COL             S            256A   DIM(27) INZ(*BLANKS)
     D COLPTR          S              2S 0
     D COLPOS          S              3S 0
     D CHRBUF          S              1A
     D HEXBUF          S              1A
...
|||| C                   LEAVE
|||+-C                   ENDIF
|||+-C                   IF        %TRIM(COL(27))='Monto_Pendiente_Embargo'
|||| C                   LEAVE
|||+-C                   ENDIF
|||+-C                   IF        COLPTR=27 AND CHRBUF = ' '
|||| C                   LEAVE
|||+-C                   ENDIF
||+--C                   ELSE
|||+-C                   IF        HEXBUF = X'0D' OR HEXBUF='0A'
...
||   C                   EXSR      GETCHR
     C                   IF        COLPTR = 6
     C                   IF        COL(COLPTR) = 'METAL' AND CHRBUF = '@'
||   C                   EXSR      GETCHR
     C                   ENDIF
     C                   IF        COL(COLPTR) = 'R' AND CHRBUF = '@'
||   C                   EXSR      GETCHR
     C                   ENDIF
     C                   ENDIF
|+---C                   ENDDO
