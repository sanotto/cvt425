...
|GDE C                   MOVE      AFIDPT        WWIDPT
     ...
     C     A#ICIV        IFNE      HJICIV
     C                   Z-ADD     A#ICIV        HJICIV
     C     HJICIV        CHAIN     BACIVA                             80
     C                   ENDIF
     C                   Z-ADD     1             WWITDM
...
GDE  C*
     ...
     C     @KEY11        CHAIN     REBAICCL                           80
     C                   DOW       *IN80 = *OFF
     C                   ADD       1             WWCAIN
     C     @KEY11        READE     REBAICCL                               80
     C                   ENDDO
     C/FREE
     ...
     C/FREE
        ...
        Write REBAHILI;
        Eval HLDES3='      <NOMB_INTEGRANTE>'+%TRIM(A#NYAP)+
                          '</NOMB_INTEGRANTE>';
        Write REBAHILI;
        Eval HLDES3='      <CUIL_INTEGRANTE>'+%TRIM(%EDITC(A#ICUI:'Z'))+
                          '</CUIL_INTEGRANTE>';
        Write REBAHILI;
     ...
     C/EXEC SQL
     C+   SELECT
     C+       IFNULL(SUM(CASE WHEN CEIMCC = 21  THEN GD$IMP ELSE 0 END), 0)
     C+       AS TOTIVA,
     C+       IFNULL(SUM(CASE WHEN CEIMCC = 550 THEN GD$IMP ELSE 0 END), 0)
     C+       AS TOTLEY,
     C+       IFNULL(SUM(CASE WHEN CEIMCC = 554 THEN GD$IMP ELSE 0 END), 0)
     C+       AS TOTIBR,
     C+       IFNULL(SUM(CASE WHEN CEIMCC = 555 THEN GD$IMP ELSE 0 END), 0)
     C+       AS TOTSIR,
     C+       IFNULL(SUM(CASE WHEN CEIMCC = 551 OR
     C+                            CEIMCC=552 THEN GD$IMP ELSE 0 END), 0)
     C+       AS TOTCRE
     C+       INTO
     C+       :TOTIVA,
     C+       :TOTLEY,
