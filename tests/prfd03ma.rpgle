*SRCMBRTXT:Aut.Federal-Manejador de Recepción de a
     ...
     FPRAFED05  UF   E           K DISK    RENAME(REPRAFED:R1)
     F*----------------------------------------------------------------*
     FSGSYSV    IF   E             DISK
...
+----c                   DOW       ec>=0 or  line <> *blanks
     ...
     c                   Move      *blanks       chdise            6
     c                   Move      *blanks       chdisd            2
     c                   Z-Add     *Zero         wwdisp            8 2
...
|    C                   Eval      chhau1=%subst(chhful:1:2)+
     ...
     c                                    '00'
     c*
     c                   Eval      chdise=%subst(line:163:006)
     c                   Eval      chdisd=%subst(line:170:002)
     c                   MoveL     chdise        wwdisp
     c                   Move      chdisd        wwdisp
...
|    C                   Eval      chfe01=%subst(chfful:7:4)+
     ...
     c                   Move      'A'           WWESTA
     C                   EndIf
     c                   If        %subst(line:150:003)='DUP'
     c                               and AASFEI = %int(chfech)
     c                   Move      'A'           WWESTA
     c                   Eval      WWAMRC=%trim(%subst(line:172:050))+' (batch)'
     c                   ExSr      ProcDup
     C                   ELSE
...
+----C                   ENDDo
     ...
     c*
     C                   Call      'PRFD04MA'
     C                   Parm      *BLANKS       METODO            6
     C                   Parm      *BLANKS       WWIRRN           15 0
...
+----C                   EndIf
     ...
     C                   UNLOCK    PRAFED05
     c*
     C     KFD0101       Chain     R1                                 99
     C                   If        *IN99 = *OFF
     c                   Move      WWESTA        AFESTA
     c                   Eval      AFAMRC=%TRIM(AFAMRC)+' (batch)'
     C*
     C                   Update    R1
     C                   EndIf
     c*
