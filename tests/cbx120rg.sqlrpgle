...
//...Consulta para listar todo el contenido de la carpeta Presentaciones.
        exec sql DECLARE C2 CURSOR FOR
         SELECT
               CAST (FILENAME as char(255)) AS FILNAM
         FROM  TABLE(IFSDIR('/home/tmp/Presentaciones/descargas')) AS T;
...
//...Traigo archivos del servidor y ejecuto comando con un sbmjob
    comando_scp = 'scp -P 998 -r '              +
                   %TRIM(url)                   +
                  ':'                           +
                   %trim(carpeta)               +
                  'files_cubix/descargas/ '     +
                  '/home/tmp/Presentaciones > ' +
...
//...Borro los archivo de la carpeta descargas del servidor
    comando_ssh = 'ssh -p 998 '                 +
                  %TRIM(url)                    +
                  ' "rm -r '                    +
                  %trim(carpeta)                +
                  'files_cubix/descargas/*" > ' +
...
//...Muevo archivo SUELDO de presentaciones a acreditaciones.
         cmd = 'mv /home/tmp/Presentaciones/descargas/' +
