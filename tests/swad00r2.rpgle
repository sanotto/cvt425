*SRCMBRTXT:Switch-Adapter      -Movs C.A.         
     ...
     C                   EXSR      ENDPGM
     C*-------------------------------------------------------------------------
     C* GetComLim: Obtenere comisiones y límites según el beneficio
     C*-------------------------------------------------------------------------
     C     GetComLim     BEGSR
     c*
     c                   Z-Add     100           ImpMinParaComi   15 2
     c                   Z-Add     2             WWIEMP
     C     WKEY01        CHAIN(n)  REANLIHA                           80
     c                   If        *In80 = *Off
     c                   If        %subst(ANRENG:23:4) = 'SUAF' Or
     c                             %subst(ANRENG:23:4) = 'UVHI'
     c                   Z-Add     50            ImpMinParaComi   15 2
     c                   Z-Add     1             WWIEMP
     c                   EndIf
     c                   EndIf
     c*
     ...
     c                             'SWAD00R2 Se Cancela pues Recibio:'+
     c                             @PEXEP +
     c                             @PEXNO +
     c                             ''') TOUSR(*SYSOPR)'
     c                   Call      'QCMDEXC'
