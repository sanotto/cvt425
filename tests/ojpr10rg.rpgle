*SRCMBRTXT:OJ-Import Res. Internet-Lector de Archi
     ...
     D COL             S            256A   DIM(12) INZ(*BLANKS)
     D COLPTR          S              2S 0
     D COLPOS          S              3S 0
     D CHRBUF          S              1A
     D HEXBUF          S              1A
 ...
 >   D RESSAL          S             15S 2
     ...
     C     GETINF        BEGSR
     C                   RESET                   COL
>    C                   FOR       COLPTR= 1 TO 12
     C                   EXSR      GETCOL
     C                   IF        INFEOF
     C                   SELECT
 >   C                   WHEN      (COLPTR > 1 AND COLPTR < 11)
     C                   WRITE     ERRUNEEOF
     C                   EVAL      IMPOKY='N'
     C                   EXSR      ENDPGM
     C                   WHEN      COLPTR = 1
  ...
  >  C                   WHEN      COLPTR = 11
     ...
     C                   EVAL      RESINS=VAL(COL(3))
     C                   EVAL      RESNOF=VAL(COL(4))
     C                   EVAL      RESTOF=%SUBST(COL(5):1:2)
     C                   EVAL      RESTID=VAL(COL(6))
     C                   EVAL      RESNDO=VAL(COL(7))
     C                   EVAL      RESCTA=%SUBST(COL(8):1:20)
     C                   EVAL      RESMOR=VAL(COL(9))
     C                   EVAL      RESMEN=VAL(COL(10))
   > C                   EVAL      RESSAL=VAL(COL(11))
     C                   EVAL      RESOBS=%SUBST(COL(12):1:15)
     C                   EVAL      RENNRO=RENNRO+1
     C                   Z-ADD     RESINS        OJININ
     C                   Z-ADD     RESNOF        OJINUI
     C                   Z-ADD     RESNDO        OJIINI
     C                   Z-ADD     RESTID        OJNTDO
     C                   Z-ADD     RESFEC        MRFCIM
   > C                   Z-ADD     RESSAL        OJ$SAL
     C   OV              WRITE     LSTTIT
     C                   WRITE     LSTDET
