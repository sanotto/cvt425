*SRCMBRTXT:Switch-Adapter      -Envio de PBF a Swi
     ...
     c                   MoveL(P)  '39.0.1.24'   host
     c                   Move      'PR00525'     user             10
     c                   Move      'sdfghjk9'    pass             10
     c                   Move      '39.0.1.7'    retsvr           15
     c                   Move      '0317'        wwmmdd            4
     c                   Move      *Blanks       path             77
     c                   Eval      path='/QDLS/SCLT000/'
     c                   Z-Add     50028         WWPORT            7 0
     ...
     C*************************************************
     c                   z-add     *Zero         retries           3 0
     C                   dow       connect(sock: p_connto: addrlen) < 0 and
     c                             retries < 3
     c                   add       1             retries
     c                   callp     close(sock)
     c                   Eval      cmdstr='SNDMSG MSG(''El switch  no respon' +
     c                                    'de, rearranque el programa de ate' +
     c                                    'nción desde el monitor del switch'''+
     C                                    ') TOUSR(*SYSOPR)'
     c                   call      'QCMDEXC'
     c                   parm                    cmdstr
     c                   parm      2048          cmdlen
     c                   Eval      cmdstr='DLYJOB DLY(120) '
     c                   call      'QCMDEXC'
     c                   parm                    cmdstr
     c                   parm      2048          cmdlen
     c                   enddo
     c
     C                   if        connect(sock: p_connto: addrlen) < 0 and
     c                             retries >= 3
     c                   Eval      cmdstr='SNDMSG MSG(''El switch  no respon' +
     c                                    'dio ejecute procedimiento manual''' +
     C                                    ') TOUSR(*SYSOPR)'
     c                   call      'QCMDEXC'
     c                   parm                    cmdstr
     c                   parm      2048          cmdlen
