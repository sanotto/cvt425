*SRCMBRTXT:LINK-Crea archivo de Cambio de Tarjetas
     ...
     c                   Move      *Blanks       Buf
     C*
     C                   MOVE      *ZEROS        TARVIE           16
     C                   MOVE      *ZEROS        TARVIV            1
     C                   MOVE      *ZEROS        TARVIG            1
     C                   MOVEL     C1NTAR        TARVIE
     C                   MOVEL     C1IVIN        TARVIV
     C                   MOVEL     C1IDIG        TARVIG
     C*
     C                   MOVE      *ZEROS        TARNUE           16
     C                   MOVE      *ZEROS        TARNUV            1
     C                   MOVE      *ZEROS        TARNUG            1
     C                   MOVEL     C1ITAR        TARNUE
     C                   MOVEL     C1IDVE        TARNUV
     C                   MOVEL     C1IDVA        TARNUG
     C*
     C                   Eval      %subst(buf:001:004)='0309'
     C                   Eval      %subst(buf:005:016)=TARVIE
     C                   Eval      %subst(buf:021:001)=TARVIV
     C                   Eval      %subst(buf:022:001)=TARVIG
     C                   Eval      %subst(buf:023:002)='  '
     C                   Eval      %subst(buf:025:004)='0309'
     C                   Eval      %subst(buf:029:016)=TARNUE
     C                   Eval      %subst(buf:045:001)=TARNUV
     C                   Eval      %subst(buf:046:001)=' '
     C                   Eval      %subst(buf:047:002)='  '
     C*
     c                   CALLP     write(FilHnd: %addr(buf   ): %size(buf   ))
     c*
     c                   EndSr
     ...
     c                   Move      *Blanks       Buf
     C*
     c                   Move      Count         CHCount           8
     C*
     C                   Eval      %subst(buf:001:002)='FT'
     C                   Eval      %subst(buf:003:008)=CHCount
     C*
     c                   CALLP     write(FilHnd: %addr(buf   ): %size(buf   ))
     c*
     C                   EndSr
