*SRCMBRTXT:BAHILI: Imprimir Hoja De Resumen       
     ...
     C                   Move      *Off          EnviaLink         1
     c                   If        PAMODE='1'
     c* ... det dir email usuario sideba
     c/exec sql
     C+ SELECT CVIQOU INTO :Destino FROM SGUSUA WHERE CVIUSR = :@PUSER
     c/end-exec
     C                   Move      *On           EnviaLink         1
     c                   ExSr      EnviarCorreoI
     c                   Else
     c* ... det dir email cliente (HACER SQL CORRESPONDIENTE)
     c/exec sql
     C+ SELECT CVIQOU INTO :Destino FROM SGUSUA WHERE CVIUSR = :@PUSER
     c/end-exec
     c                   ExSr      EnviarCorreoE
     c                   Endif
     C                   EndSr
     c*---------------------------------------------------------------------
     C     EnviarCorreoI BegSr
     c*---------------------------------------------------------------------
     c*
     c*
      /free
     ...
     c                   EndSr
     c*---------------------------------------------------------------------
     C     EnviarCorreoE BegSr
     c*---------------------------------------------------------------------
     c* ... lo mismo pero con el comando sndgmail , al comando sndgmail
     c*     hay que agregarle un parametro mas
     c                   EndSr
     c*---------------------------------------------------------------------
