...
||   C     WKEY01        READE     REPRHAMV                               99
|+---C                   ENDDO
|    C* ... Aplicar de Mayor a Memor MISMA AREA MISMO TRAB
...
|    C/END-EXEC
|    C/EXEC SQL
|    C+ UPDATE PRHAMV SET MVCREC = 0, MV$IMO = 0, MV$ICU = 0,
|    C+ MV$IDP = 0, MV$SBL = 0 WHERE MVFAAM=:WWFAAM AND MVIEMP=:WWIEMP
|    C/END-EXEC
|    C                   ENDSR
     C*-------------------------------------------------------------------------
     C* CHKFIL : Chequea cantidad de filas e importes
     C*-------------------------------------------------------------------------
     C     CHKFIL        BEGSR
     C                   Z-ADD     *ZERO         CKIMPO           15 2
     C                   Z-ADD     *ZERO         CKCANR           10 0
     C*
     C/EXEC SQL
     C+ SELECT SUM(COD570+COD571+ COD754+COD755), COUNT(*) INTO :CKIMPO,
     C+ :CKCANR FROM QTEMP/PRHAVU
     C/END-EXEC
     C*
     C                   IF        WWIMPO <> CKIMPO AND
     C                             WWCANR <> CKCANR
     C                   EVAL      PADESC='NO COINCIDEN CANT REG O IMPORTE'
     C                   EXSR      ENDPGM
     C                   ENDIF
     C*
     C                   ENDSR
...
|    C/EXEC SQL
     ...
     C+  SEXO,
     C+ SUM(COD570+COD571+ COD754+COD755), SUM(0) FROM QTEMP/PRHAVU GROUP BY
