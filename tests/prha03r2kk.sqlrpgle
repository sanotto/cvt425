*SRCMBRTXT:Ces.Hab.-Validación    -Valida  Pgm Pri
     H DEBUG DATEDIT(*YMD)
     H*****************************************************************
     H*                                                               *
...
+----C                   ENDDO
     ...
     C                   EXSR      ENDPGM
     C*-------------------------------------------------------------------------
     C* PRCCRE1: Proceso Créditos solo iguales
     C*-------------------------------------------------------------------------
     C     PRCCRE1       BEGSR
...
|+---C                   ENDDO
     C                   ENDSR
     C*-------------------------------------------------------------------------
     C* PRCCRE : Proceso Créditos solo iguales
     C*-------------------------------------------------------------------------
     C     PRCCRE2       BEGSR
     C                   Z-ADD     WW$BAN        WW$IMO
|||| C                   EXSR      SAVIMO
...
||   C     WKEY01        READE     REPRHAMV                               99
|+---C                   ENDDO
|    C* ... Aplicar de Mayor a Memor y viceversa MISMA AREA MISMO TRAB
|    C                   EVAL      WW$SA0=WW$BAN
|    C     WKEY01        CHAIN     REPRHAMV                           99
|+---C                   DOW       *IN99 = *OFF
||+--C                   IF        MVCREC=0  OR MVCREC=2
|||+-C                   IF        MVEMSU <> 3
|||| C                   IF        MVIBCF = %XLATE(lo:up:AREA)   AND
     C                             MVIRED =TRAB
|||| C                   IF        MV$IMP <= WW$SA1
...
|+---C                   WHEN      (WW$SA0 <> 0 AND
     C                              WW$SA1 <> 0 AND
     C                              WW$SA2 <> 0       ) AND
||   C                              WW$SA1 <= WW$SA2 AND WW$SA1 <=WW$SA0
||   C                   EXSR      MAYMEN
|+---C                   WHEN      (WW$SA0 <> 0 AND
     C                              WW$SA1 <> 0 AND
     C                              WW$SA2 <> 0       ) AND
||   C                              WW$SA2 <= WW$SA1 AND WW$SA2 <=WW$SA0
||   C                   EXSR      MENMAY
|+---C                   WHEN      (WW$SA0 <> 0 AND
     C                              WW$SA1 <> 0 AND
     C                              WW$SA2 <> 0       ) AND
||   C                              WW$SA0 <= WW$SA1 AND WW$SA0 <=WW$SA2
||   C                   EXSR      MAYMENIGAREA
|+---C                   OTHER
||   C                   EXSR      MAYMEN
|+---C                   ENDSL
...
|    C     MARVUE        BEGSR
     C*
     C*EXEC SQL
     C* UPDATE PRHAVU SET HORA = '999' WHERE RRN(PRHAVU)=:RRN
     C*END-EXEC
     C*
     C                   ENDSR
|    C*----------------------------------------------------------------
...
|    C     WKEY01        CHAIN     REPRHAMV                           99
|+---C                   DOW       *IN99 = *OFF
     C                   IF        MVCREC = 1 AND MV$ICU < MV$IMP AND
     C                             MVIBCF = %XLATE(lo:up:AREA)
     C     MV$IMP        SUB       MV$ICU        WWDIFE           15 2
     C                   IF        WWDIFE <= WW$BAN
     C                   ADD       WWDIFE        MV$ICU
     C                   UPDATE    REPRHAMV
     C                   EVAL      WW$BAN=WW$BAN-WWDIFE
     C                   ELSE
     C                   ADD       WW$BAN        MV$ICU
     C                   UPDATE    REPRHAMV
     C                   EVAL      WW$BAN=0
...
|    C     WKEY01        CHAIN     REPRHAMV                           99
|+---C                   DOW       *IN99 = *OFF
     C                   IF        MVCREC = 1 AND MV$ICU < MV$IMP
     C     MV$IMP        SUB       MV$ICU        WWDIFE           15 2
     C                   IF        WWDIFE <= WW$BAN
     C                   ADD       WWDIFE        MV$ICU
     C                   UPDATE    REPRHAMV
...
|||| C  N99              EVAL      MVITII=ARITIN
|||| C  N99              EVAL      MVINUI=ARININ
|||| C                   ENDIF
|||| C                   ELSE
...
|    C                   EVAL      SQLCOD=*ZERO
     C/EXEC SQL
     C+ UPDATE PRHAVU SET HORA ='000'
     C/END-EXEC
|    C/EXEC SQL
     C+ DECLARE C1 CURSOR FOR SELECT                     AREA, DOCU , NOMB,
     C+  SUBSTR('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ',
     C+   INT(CAST(TRAB AS DEC(2, 0)))+1, 1) TRAB,
     C+  SEXO,
     C+ SUM(COD570+COD571+ COD754+COD755), SUM(0) FROM PRHAVU GROUP BY
     C+                              AREA, DOCU, NOMB, SEXO, TRAB ORDER BY
     C+ SUM(COD570+COD571+COD754+COD755) DESC , SUM(0) DESC
|    C/END-EXEC
|+---C                   IF        SQLCOD <> 0
...
|    C                   ENDSR
|    C*-------------------------------------------------------------------------
|    C* OPNCUR2: Abrir Cursor
|    C*-------------------------------------------------------------------------
|    C     OPNCUR2       BEGSR
|    C                   EVAL      SQLCOD=*ZERO
|    C/EXEC SQL
     C+ DECLARE C2 CURSOR FOR SELECT RRN(PRHAVU), AREA, DOCU , NOMB,
     C+  SUBSTR('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ',
     C+   INT(CAST(TRAB AS DEC(2, 0)))+1, 1) TRAB,
     C+  SEXO,
     C+ SUM(COD570+COD571+ COD754+COD755), SUM(0) FROM PRHAVU
     C+
     C+ GROUP BY
     C+ AREA, DOCU, NOMB, SEXO, TRAB ORDER BY
     C+ SUM(COD570+COD571+COD754+COD755) DESC , SUM(0) DESC
...
|    C     FETNEX        BEGSR
|    C/EXEC SQL
|    C+ FETCH NEXT FROM C1 INTO :RRN, :AREA, :DOCU, :NOMB, :TRAB, :SEXO,
|    C+ :WW$BAN, :WW$CRE
|    C/END-EXEC
|    C
...
|    C                   ENDSR
|    C*-------------------------------------------------------------------------
|    C* FETNEX2: Leer Proxima Fila del Cursor
|    C*-------------------------------------------------------------------------
|    C     FETNEX2       BEGSR
|    C/EXEC SQL
|    C+ FETCH NEXT FROM C2 INTO :RRN, :AREA, :DOCU, :NOMB, :TRAB, :SEXO,
|    C+ :WW$BAN, :WW$CRE
|    C/END-EXEC
|    C
|    C* ... Si es un doc duplicado renormalizar a 90
...
|    C                   ENDSR
|    C*-------------------------------------------------------------------------
|    C* CRERIO : Credito Riojano
|    C*-------------------------------------------------------------------------
|    C     CRERIO        BEGSR
|    C     WKEY01        CHAIN     REPRHAMV                           99
|+---C                   DOW       *IN99 = *OFF
||+--C                   IF        MVEMSU=3
|||  C                   EVAL      MV$IMO=MV$IMO+WW$CRE
|||  C                   EVAL      MV$SBL=MV$SBL+WW$CRE
|||  C                   EVAL      WW$CRE= 0
|||  C                   EVAL      MVCREC = 2
|||  C                   UPDATE    REPRHAMV
||+--C                   ENDIF
