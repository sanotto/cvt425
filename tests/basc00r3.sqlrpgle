...
||   c                   EVAL      PA$IMP= WK$SAL
     c*                  IF        TMIGRC='06'
     C* GRUPOS 01, 02, 06, 07 MASTERCARD. GRUPO 03 VISA
     c     TMIGRC        IFNE      '03'
||   C                   EXSR      ADD02491
     c                   ELSE
||   C                   EXSR      ADD02392
...
+----C                   ENDDO
     ...
     C                   PARM                    WWICUO
     C                   PARM                    WW$IMP
     C* ... Actualiza campo Operacion Anterior (Efectivamente Cancelada)
     C     @PJOBN        CHAIN     @CPIUSD
     C                   Z-ADD     C1SUCA        DSSUCA
     C                   Z-ADD     C1INCR        DSINCA
     C                   Z-ADD     DSOPAN        @CINCV
...
+----C+                  WHEN ((
     C+                             (SCDF05='S/RECIBO')
     C+          OR (SCDF05 IS NULL AND TB$A03 =1 AND TBIEMP <> 500)
     C+                        )
     C+                        AND NOT
     C+                        (     JVISUC=:RFSUCA
     C+                          AND JVINCR=:RFINCR
     ...
     C+            (SCDF05 IS NULL  AND TB$A03=1 AND TBIEMP <> 500 )
     C+                                 )
     C+                                 AND NOT
     C+                                 (     JVISUC= :RFSUCA
     C+                                   AND JVINCR =:RFINCR
