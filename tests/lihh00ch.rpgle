*SRCMBRTXT:Link-Host to Host-Procedimientos de Can
     ...
     C                   ExSr      OpenServer
     C                   EndIf
     C                   CallP     LogError('INF':'Arrancada Conex.'+ConnNme )
     C                   Return    *Off
     C*-----------------
     ...
     C*
     C     *LOVAL        SetLL     RELIFLDS
     C                   Read      RELIFLDS                               99
     C                   DoW       *IN99 = *OFF
     C                   Eval      mBitsXLong(IFNFLD)=IFCBIT
     C                   Eval      mDtaType(IFNFLD)  =IFTDAT
     C                   Eval      mLongMax(IFNFLD)  =IFLONG
     C                   Eval      mLongMin(IFNFLD)  =IFNPOI
     C                   Read      RELIFLDS                               99
     C                   EndDo
     C*
     C                   EndSr
     ...
     P CloseChanel     B
     D CloseChanel     PI
     C                   CallP     LogError('INF':'Cerrada Conex.'+sConnName)
     C                   callp     close(csock)
     C                   callp     close(lsock)
     P CloseChanel     E
      *-------------------------------------------------------------------------
     ...
     C                                             : msglen    : *On)
     C* ... ... Si pudo Leer el mensaje completo...
     C                   If         rc <> msglen
     C                   CallP     LogError('ERR':'RdLn:'+%str(strerror(errno)))
     C                   Return    *On
     C                   EndIf
     C                   Else
     C                   CallP     LogError('ERR':'RdLn:'+%str(strerror(errno))+
     C                             '-Mensaje Incompleto')
     C                   EndIf
     C                   Return    *Off
     P ReadChanel      E
      *-------------------------------------------------------------------------
     ...
     C                   MoveL(P)  sConnName     RLORGN
     C                   MoveL(P)  errorText     RLMSJE
     C                   Write     RLINKF
     P LogError        E
