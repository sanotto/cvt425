...
dcl-proc main;
   ...
   // end-pi;
    ...
    dcl-s codigo_error2     char(1) inz('0');
    dcl-s nro_oficio        char(11);
    dcl-s nro_cuit          char(13);
    dcl-s razon_social      char(30);
...
// Otras variable
    ...
    dcl-s  WWIFEI      char(20);
    dcl-s  WWISRE      packed(15:2);
    dcl-s  WWIDSA      packed(15:2);
    dcl-s  WWTIPO      char(2);
...
//... Llamada al programa 'OJIM15CL'
   dcl-pr callOjim15cl extpgm('OJIM15CL');
    end-pr;
...
//... Llamada al programa 'WSSOJ0R2'
 dcl-pr callSRWSSOJ0R2 extpgm('WSSOJ0R2');
...
//... Verifica si el numero de oficio ya fue procesado
      EXSR verificaMarca;
...
//=========================================================================
 ...
 begsr Generar_archivo_de_respuestas;
      ...
      EXSR verificar_Ojmres;
       ...
       cmd2 = 'SNDGMAIL TOUSR('+%trim(notify_address)+') '              +
               ' SUBJECT(''Aviso: SOJ Archivo Respuesta'') '            +
               ' BODY(''No se encontraron registros para la fecha '     +
                             %trim(chsfei)                              +
               ' en el archivo OJMRES. '                                +
               'No se genero el archivo de respuesta'                   +
               ' '') '                                                  ;
...
//... Select al ojmofi para traer todos los oficios de manera online
...
//... Select al ojmofi para saber si ya fue procesado en el remanente

exec sql declare ojm_marca cursor for
    SELECT OJINUI as nro_oficio
    FROM   OJMOFI
    WHERE
           OJINUI = :nro_oficio
       AND OJDF03 = '1'
...
//... Select al ojmofi para saber si ya fue reversado

exec sql declare ojm_marca_r cursor for
    SELECT OJINUI as nro_oficio
    FROM   OJMOFI
    WHERE
           OJINUI = :datos_ojmofi.nro_oficio
       AND OJDF04 = :WWIFEI ;
...
exec sql declare tipoPersona cursor for
    SELECT A#INDO as nro_cuit
    FROM   BAPFIS
    WHERE
           A#ICUI = :cuitSinGuiones
...
exec sql declare tipoReversa cursor for
       SELECT    U1ISUB
       FROM      BAWSSJ
       WHERE
                 U1FVGD = :datos_ojmofi.nro_iche;


//... Select para obtener la suma total de los movimientos reversados AC

exec sql declare movAC cursor for
       SELECT    SUM(GC$IMP)
       FROM      ACMOVD
       WHERE
                 GCICHE = :datos_ojmofi.nro_iche
            AND  GCIGRC <> '05'
            AND  GCISGC <> 'OJ'
            AND  GCIASC = 1
            AND  GCIMCA = 788
...
//... Select para obtener la suma total de los movimientos reversados CC

exec sql declare movCC cursor for

       SELECT SUM(CF$IMP)
       FROM      CCMOCT
       WHERE
                 CFICHE = :datos_ojmofi.nro_iche
            AND  CFIGRC <> '05'
            AND  CFISGC <> 'OJ'
            AND  CFIASC = 1
            AND  CFIMCC = 788
...
//=========================================================
   //  Verifica si el oficio ya fue procesado
//=========================================================

BEGSR  verificaMarca;

     exec sql open ojm_marca;
     exec sql FETCH ojm_marca;

     If SQLCOD = *ZERO;
      codigo_error = '1';
     endif;

     exec sql close ojm_marca;

ENDSR;


//=========================================================
   //  Verifica si el oficio ya fue reversado
//=========================================================

BEGSR  verificaMarcaR;

     exec sql open ojm_marca_r;
     exec sql FETCH ojm_marca_r;

     If SQLCOD = *ZERO;
      codigo_error2 = '1';
     endif;

     exec sql close ojm_marca_r;
...
//=================================================
   //  Verifica que la cuenta sea valida persona juridica
//=================================================
...
//=================================================
   //  Verifica que la cuenta sea valida persona fisica
//=================================================
...
//=================================================
   //  Recupera el saldo reversado de AC y CC
//=================================================

BEGSR  calcularContraAsientoAC;

     exec sql open movAC;
     exec sql FETCH movAC INTO :WWISRE;

     If SQLCOD <> *ZERO;
      codigo_error2 = '1';
     endif;

     exec sql close movAC;

ENDSR;


//=================================================
   //  Calcular la suma de los movimientos reversados
//=================================================

BEGSR  calcularContraAsientoCC;

     exec sql open movCC;
     exec sql FETCH movCC INTO :WWISRE;

     If SQLCOD <> *ZERO;
      codigo_error2 = '1';
     endif;

     exec sql close movCC;

ENDSR;


//=================================================
   //  Verifica el tipo de cuenta a reversar
//=================================================

BEGSR  verificarTipoCuenta;

     exec sql open tipoReversa;
     exec sql FETCH tipoReversa INTO :WWTIPO;

     If SQLCOD <> *ZERO;
      codigo_error2 = '1';
     endif;

     exec sql close tipoReversa;
...
BEGSR modificaOjmofi;
    exec sql UPDATE OJMOFI set OJDF05 = :nro_iche
                        WHERE  OJINUI = :WWINUC;
...
//==============================================================
   //  Marcar como procesado por el archivo remanente
//==============================================================

BEGSR marcarOjmofi;
    exec sql UPDATE OJMOFI set OJDF03 = '1',
                               OJDF04 = :chsfei
                        WHERE  OJINUI = :nro_oficio;

ENDSR;


//==============================================================
   // Borrar el ojmofi de acuerdo a los movimientos reversados
//==============================================================
...
//==============================================================
   // Borrar el ojmres de acuerdo a los movimientos reversados
//==============================================================
...
//==============================================================
   //  Modifica el saldo reversado del oficio
//==============================================================

BEGSR modificaOjmofiReversa;
    exec sql UPDATE OJMOFI set OJ$SAL = :WWIDSA,
                               OJDF04 = :aasfei
                        WHERE  OJINUI = :datos_ojmofi.nro_oficio;
...
BEGSR modificaOjmres;
    exec sql UPDATE OJMRES set MR$ICU = :WWIDSA
                        WHERE    MRINUI = :datos_ojmofi.nro_oficio
                             AND MRFALT = :aasfei ;
...
BEGSR consultarContraAsiento;
   ...
   //Aca obtengo los datos del ojmofi para comparar
    ...
    DoW sqlcod = *Zero;
    codigo_error2 = '0';
    EXSR verificarTipoCuenta;
    EXSR verificaMarcaR;

    if WWTIPO = 'AC';

    EXSR calcularContraAsientoAC;

    else;

    EXSR calcularContraAsientoCC;

    endif;

    if codigo_error2 = '0';

     if datos_ojmofi.saldo = WWISRE;
      ...
      if datos_ojmofi.saldo > WWISRE;
         ...
         WWIDSA = datos_ojmofi.saldo - WWISRE;

      else;

         WWIDSA = WWISRE - datos_ojmofi.saldo;

      endif;

       EXSR modificaOjmofiReversa;
       EXSR modificaOjmres;
...
//=================================================================
   //  Llama al programa ojim15cl para enviar los reportes por mail
//=================================================================
   BEGSR EnviarReportes;
