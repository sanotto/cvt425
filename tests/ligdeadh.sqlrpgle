    ...
    begsr insertar_cuentas_maestro_adhsesion;
        ...
        exec sql
               ...
               SELECT
                    ...
                    CASE
                        WHEN SUBSTRING(arch_adh_serv, 18, 2) ||
                             SUBSTRING(arch_adh_serv, 23, 7) = FUICAH
                        THEN 'AC'
                        WHEN SUBSTRING(arch_adh_serv, 18, 2) ||
                             SUBSTRING(arch_adh_serv, 23, 7)= BMICCC
                        THEN 'CC'
                    ELSE '-' END,
                    CASE
                        WHEN SUBSTRING(arch_adh_serv, 18, 2) ||
                ...
                LEFT JOIN ACCTAC ON FUICAH = SUBSTRING(arch_adh_serv, 18, 2) ||
                                             SUBSTRING(arch_adh_serv, 23, 7)
                                AND FUFBAJ = 0
                LEFT JOIN CCCTCT ON BMICCC = SUBSTRING(arch_adh_serv, 18, 2) ||
                                             SUBSTRING(arch_adh_serv, 23, 7)
                                AND BMFBAJ = 0
                LEFT JOIN LIGDEA ON LGICCL = SUBSTRING(arch_adh_serv, 18, 2) ||
                                             SUBSTRING(arch_adh_serv, 23, 7)
                WHERE
    ...
    begsr actualizar_bajas_maestro_adhesion;
        ...
        exec sql
                ...
                WHERE NOT EXISTS(
                               SELECT *
                               FROM QTEMP.GDERMC
                               WHERE SUBSTRING(arch_adh_serv, 18, 2) ||
                                     SUBSTRING(arch_adh_serv, 23, 7) = LGICCL);
