*SRCMBRTXT:LINK:TRANSF. ENTRE REDES-GENERA PLANO  
     ...
     c                   If        TMIASK='B'
     c                   CALLP     write(FilHndD: %addr(buf   ): %size(buf   ))
     C                   Add       1             CountD
     c                   EndIf
     c                   If        TMIASK='A'
     c                   CALLP     write(FilHndA: %addr(buf   ): %size(buf   ))
     C                   Add       1             CountA
     c                   EndIf
     c                   If        TMIASK='M'
     c                   CALLP     write(FilHndM:%addr(buf   ): %size(buf   ))
     C                   Add       1             CountM
     c                   EndIf
     C                   Add       1             Count
     C*
     ...
     C                   Z-Add     CountD        CanRegD
     C                   Z-Add     CountA        CanRegA
     C                   Z-Add     CountM        CanRegM
     C                   ExSr      EndPgm
     C*-------------------------------------------------------------------------
     ...
     c                   CALLP     write(FilHnd : %addr(buf   ): %size(buf   ))
     C                   Add       2             CountD
     C                   Move      CountD        CNTCHRD           9
     C                   Eval      %subst(buf:006:009)=CNTCHRD
     C                   Eval      %subst(buf:015:204)=*BLANKS
     c                   CALLP     write(FilHndD: %addr(buf   ): %size(buf   ))
     C                   Add       2             CountA
     C                   Move      CountA        CNTCHRA           9
     C                   Eval      %subst(buf:006:009)=CNTCHRA
     C                   Eval      %subst(buf:015:204)=*BLANKS
     c                   CALLP     write(FilHndA: %addr(buf   ): %size(buf   ))
     C                   Add       2             CountM
     C                   Move      CountM        CNTCHRM           9
     C                   Eval      %subst(buf:006:009)=CNTCHRM
     C                   Eval      %subst(buf:015:204)=*BLANKS
     c                   CALLP     write(FilHndM: %addr(buf   ): %size(buf   ))
     C*
     C                   EndSr
     ...
     c*
     C*
     c                   EVAL      FilHndA=open(%trim(txtfileA):
     c                                  O_CREAT + O_WRONLY + O_TRUNC +
     c                                  O_CODEPAGE :
     c                                  S_IWUSR+S_IRUSR+S_IRGRP+S_IROTH:
     c                                  819     )
     c                   callp     close(FilHndA)
     c                   EVAL      FilHndA=open(%trim(txtfileA):
     c                                   O_WRONLY+O_TEXTDATA)
     c*
     C*
     c                   EVAL      FilHndM=open(%trim(txtfileM):
     c                                  O_CREAT + O_WRONLY + O_TRUNC +
     c                                  O_CODEPAGE :
     c                                  S_IWUSR+S_IRUSR+S_IRGRP+S_IROTH:
     c                                  819     )
     c                   callp     close(FilHndM)
     c                   EVAL      FilHndM=open(%trim(txtfileM):
     c                                   O_WRONLY+O_TEXTDATA)
     c*
     C                   EndSr
