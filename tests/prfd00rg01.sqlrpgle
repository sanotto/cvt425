*SRCMBRTXT:Aut.Federal-Inserta pedido de Aut. en P
     ...
     c                   ExSr      EndPgm
     c*-------------------------------------------------------------------------
     c* LlamoWebService: Ejecuto consulta FEDERAL vía web services
     c*-------------------------------------------------------------------------
     c     LlamoWebServ  BegSr
     c*
     C                   Move      *BLANKS       W1ERRO            1
     C                   Move      *BLANKS       W1MSGE          216
     c                   Call      'PRFD20MA'
     c                   Parm                    AFIRRN
     c                   Parm                    AASFEI
     c                   Parm                    W1ERRO
     ...
     C*
     c                   If        W1ERRO = '1'
     c                   Eval      WWNCU1='Error al comunicar con Federal'
     c                   Eval      WWNCU2='Op. sujeta a aprobacion por archivo'
     C                   Eval      WWNCB4 = %subst(W1MSGE:163:54)
     c                   ExSr      DspErr
     c                   Else
     C                   Eval      WWNCU1 = %subst(W1MSGE:1:54)
     C                   Eval      WWNCU2 = %subst(W1MSGE:55:54)
     C                   Eval      WWNCU3 = %subst(W1MSGE:109:54)
     c                   ExSr      DspErr
     C                   EndIf
     c*
     c                   EndSr
...
|+---C                   ENDIF
     ...
     C                   EVAL      MCIAPC=1
     C                   EVAL      MCITRN=210
     C*RESGUARDA VALORES DE CPI
     C     @PJOBN        Chain(N)  @CPIUSD                            80
     c                   Z-ADD     @CICMR        WZICMR           20 0
     c                   MOVEL     @CNRSO        WZNRSO           30
     c                   Z-ADD     @CICUI        WZICUI           12 0
     c                   Z-ADD     @CIRUR        WZIRUR            1 0
     c                   MOVEL     @CISUB        WZISUB            2
     c                   Z-ADD     @CISUC        WZISUC            5 0
     c                   Z-ADD     @CICCL        WZICCL            9 0
     c                   Z-ADD     @CIDVA        WZIDVA            1 0
     c                   Z-ADD     @CIDVE        WZIDVE            1 0
     c                   MOVEL     @CIUSR        WZIUSR           10
     c                   Z-ADD     @CFALT        WZFALT            8 0
     c                   Z-ADD     @CHALT        WZHALT            6 0
     C
     C     LOOPCOM       TAG
     C
     C                   CALL      'BACO00MW'
     ...
     C                   EVAL      MCICCL=@CICCL
     C
     C*DEVUELVE  VALORES DE CPI
     C     @PJOBN        Chain     @CPIUSD                            80
     c                   Z-ADD     WZICMR        @CICMR           20 0
     c                   MOVEL     WZNRSO        @CNRSO           30
     c                   Z-ADD     WZICUI        @CICUI           12 0
     c                   Z-ADD     WZIRUR        @CIRUR            1 0
     c                   MOVEL     WZISUB        @CISUB            2
     c                   Z-ADD     WZISUC        @CISUC            5 0
     c                   Z-ADD     WZICCL        @CICCL            9 0
     c                   Z-ADD     WZIDVA        @CIDVA            1 0
     c                   Z-ADD     WZIDVE        @CIDVE            1 0
     c                   MOVEL     WZIUSR        @CIUSR           10
     c                   Z-ADD     WZFALT        @CFALT            8 0
     c                   Z-ADD     WZHALT        @CHALT            6 0
     C                   Update    @CPIUSRR
     C*
     C                   UNLOCK    @CPIUSD
     C*
