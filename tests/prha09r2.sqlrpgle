*SRCMBRTXT:Ces.Hab.-Validación Municipali-Valida  
     H DEBUG DATEDIT(*YMD)
     H*****************************************************************
     H*                                                               *
     ...
     C*
     C/EXEC SQL
     C+ SELECT SUM(IMPORTE), COUNT(*) INTO :CKIMPO, :CKCANR FROM PRHAVM
     C/END-EXEC
     C*
     ...
     C                   EVAL      SQLCOD=*ZERO
     C/EXEC SQL
     C+ DECLARE C1 CURSOR FOR SELECT CAST(NRODOC AS DEC(8, 0)) , NOMBRE,
     C+ sum(IMPORTE) FROM PRHAVM GROUP BY NRODOC, NOMBRE
     C/END-EXEC
     C                   IF        SQLCOD <> 0
