...
+----C                   EndIf
     ...
     C                                                %EDITW(
     C                                           TAIDIG:'0 '))
     c*CODIGO DE MENSAJE MSG011 certificado supervivencia
...
+----c                   endif
     ...
     c                   EndSr
     C*-------------------------------------------------------------------------
     c* wrtmsg9: Imprime codigo de mensaje MSG009
     C*-------------------------------------------------------------------------
     c     wrtmsg9       BegSr
     c*
     c*                  ExSr      CheckStruct
     c*Mensaje de ANSES MSG009, caduca el 17/09/2016
+----c     20160917      ifgt      AASFEI
|    c                   ExSr      ProcDir
+----c                   endif
     c*
     c                   EndSr
     C*-------------------------------------------------------------------------
     c* wrtmsg10: Imprime codigo de mensaje MSG010
     C*-------------------------------------------------------------------------
     c     wrtmsg10      BegSr
     c*
     c*                  ExSr      CheckStruct
     c*Mensaje de ANSES MSG010, caduca el 19/01/2017
+----c     20170119      ifgt      AASFEI
|    c                   ExSr      ProcDir10
+----c                   endif
     c*
     c                   EndSr
...
|    c                   Eval      file_name= %subst(d_name:1:d_namelen)
|    c                   Eval      file_name=%xlate(lo:up:file_name)
|    c                   eval      file_name = 'UVHI.TXT'
|    c                   ExSr      ProcessFile10
|    c                   Eval      p_dirent = readdir(p_dir)
+----c                   EndDo
     C*
     c                   CallP     closedir(p_dir)
...
|+---c                   if        mess = 'MSG009'
||   C                   Eval      indo=%subst(line:007:008)
||   c                   MOVE      INDO          NDOC
||   c                   MOVE      INDO          DOC
...
||+--c                   endif
|+---c                   else
||   c* Habilitar para mostrar mesg10
...
|||| c                   add       1             count
|||| c* MSG010
|||| c*Descomentar la linea 635 para habilitar el MSG010
|||| c
|||| c                   exsr      PrtBod4
|||| C                   move      TANTAR        WWNTAR           16 0
|||| c                   endif
...
+----C                   ENDDo
     ...
     c                   z-add     *zeros        countt           15 0
+----c                   DOW       readline(fdi: %addr(line): %size(line))>=0
|    C                   Eval      indo=%subst(line:030:008)
|    c                   MOVE      INDO          NDOC
|    c                   MOVE      INDO          DOC
|    c     @KEY04        Chain     REBADCCL                           99
|+---c                   If        *IN99 = *OFF
