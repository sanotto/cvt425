*SRCMBRTXT:Formularios: Parser de Etiquetas       
     ...
     C                   SELECT
     C                   WHEN      CurSts=0 AND chrInp   = OPNTA1
     C                   EVAL      CurSts=1
     C                   WHEN      CurSts=0 AND chrInp  <> OPNTA1
     C                   EXSR      WRTOUT
     C                   WHEN      CurSts=1 AND chrInp   = OPNTA2
     C                   EVAL      CurSts=2
     C                   EVAL      tknStr=*BLANKS
     C                   WHEN      CurSts=1 AND chrInp  <> OPNTA2
     C                   EXSR      WRTOUT
     C                   EVAL      CurSts=0
     C                   WHEN      CurSts=2 AND chrInp   = CLOTA1
     C                   EXSR      RPLDTA
     C                   EVAL      CurSts=0
     C                   WHEN      CurSts=2 AND chrInp  <> CLOTA1
     ...
     C     WKEY01        CHAIN     REBAFCPL                           25
     C                   IF        *IN25 = *OFF
     C* ... Validar Códigos de Edición
     C                   SELECT
     C                   WHEN      FLEFOR='Y'
     C                   EVAL      FLDES3= %SUBST(FLDES3:7:2)+'/'+
     C                                     %SUBST(FLDES3:5:2)+'/'+
     C                                     %SUBST(FLDES3:1:4)
     C                   WHEN      FLEFOR='J'
     C                   MOVEL     FLDES3        AUXIMP           15 2
     C                   EVAL      FLDES3=%trim(%EDITC(AUXIMP:'J'))
     C                   WHEN      FLEFOR='*'
     C                   MOVEL     FLDES3        AUXIMP           15 2
     C                   EVAL      FLDES3=%trim(%EDITC(AUXIMP:'J':*ASTFILL))
     C                   WHEN      FLEFOR='1'
     C                   MOVEL     FLDES3        AUXIMP           15 2
     C                   EVAL      FLDES3=%trim(%EDITC(AUXIMP:'1'))
     C                   WHEN      FLEFOR='0'
     C                   MOVEL     FLDES3        AUXIM2           15 0
     C                   EVAL      FLDES3=%trim(%EDITC(AUXIM2:'Z'))
     C                   ENDSL
     C* ... Convertir reemplazo a ASCII
     C                   EVAL      tknLen=%len(%trim(FLDES3))
     C     tknLen        IFGT      *ZERO
     ...
     C                   EVAL      wrkPtr=%ADDR(FLDES3)
     C                   FOR       J= 1 TO tknLen
     C                   EVAL      chrInp=%subst(FLDES3:j:1)
     C                   EXSR      WRTOUT
     C                   ENDFOR
     C                   ENDIF
     C                   ENDIF
