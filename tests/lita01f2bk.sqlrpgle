*SRCMBRTXT:TD: Acusa TD como entregada a OCA      
**free
...
dcl-proc Main;
    ...
    dcl-s error_RealizarAcuse     packed(1:0) inz(0);
    ...
    dcl-s new_tadf06              char(20);
    dcl-s fue_entregada           char(1);
...
dow bandera_salida=0;
    ...
    if bandera_salida=0;
        EXSR ValidoTD;
        if  bandera_validar_td = 0 and fue_entregada='';
            EXSR WrtLIKMTH;
            EXSR RealizarAcuse;
        else;
            EXSR ErrorValidarTD;
...
BEGSR ValidoTD;
    ...
    if sqlcode=0 and tarjeta_debito.tantar>0;
     ...
     exec sql
             SELECT SUBSTR(TADF06, 1, 1) into :fue_entregada
             FROM LIKMTR
             WHERE    TANTAR=:tarjeta_debito.tantar
                  AND TAIETA='R'
                  AND (SUBSTR(TADF06, 1, 1)='4' and
                       SUBSTR(TADF06, 2, 8) = :aasfei);
    endif;
...
BEGSR ErrorValidarTD;
    ...
    if fue_entregada = '4';
        MessageBox('La TD ya fue ACUSADA como entregada a OCA.');
    else;
        MessageBox('Estado incorrecto de la TD para realizar acuse a OCA.');
    endif;
...
BEGSR RealizarAcuse;
    ...
    new_tadf06='4'+aasfei_aaaammdd;
    exec sql
       UPDATE LIKMTR
       SET TADF06=trim(:new_tadf06)||
           (select substr(tadf06, 10, 1)
            from likmtr
            where tantar=:tarjeta_debito.tantar and taiast=0)||
            '        '||
           (select substr(tadf06, 19, 1)
            from likmtr
            where tantar=:tarjeta_debito.tantar and taiast=0)||
            ' '
       WHERE TANTAR=:tarjeta_debito.tantar;
     ...
     if sqlcode = 0;
        ...
        MessageBox('Error al realizar Acuse.');
        error_RealizarAcuse = 1;
     endif;
...
BEGSR WrtLIKMTH;
    ...
    exec sql
        ...
        SELECT
              ...
              'LITA96RG',--PROGRAMA INGRESO
              'terminal',
              'ACUSE OCA',
              'ENTREGA DE TD: ACUSE A OCA PUERTA PUERTA',
