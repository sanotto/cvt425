*SRCMBRTXT:Link-ODE-Proc Arch CSV De Formularios -
**free
ctl-opt
            option(*SRCSTMT)
            dftactgrp(*no)
...
// Variables
    ...
    dcl-s fecha_deb          packed(8:0);
    dcl-s fecha_cred         packed(8:0);
...
// Llamada al programa para debitar/acreditar caja de ahorro (SBACMOVI)
   dcl-pr callSBACMOVI extpgm('SBACMOVI');
...
// Llamada al programa para debitar/acreditar cuenta corriente  (SBCCMOVI)
   dcl-pr callSBCCMOVI extpgm('SBCCMOVI');
        sucursal      packed(5:0) CONST;
        cuenta        packed(11:0)CONST;
        fecha_alta    packed(8:0) CONST;
        hora          packed(6:0) CONST;
        moneda        packed(9:0) CONST;
        residencia    packed(2:0) CONST;
        codigo        packed(3:0) CONST;
        sucursal_alta packed(5:0) CONST;
        nro_caja      packed(5:0) CONST;
        importe       packed(15:2)CONST;
        ...
        asiento_contr packed(1:0) CONST;
        fec_asi_contr packed(8:0) CONST;
        sector_cuenta packed(4:0) CONST;
        resi_pais     packed(1:0) CONST;
        gen_asi_contr char(1)     CONST;
        usaurio1      char(8)     CONST;
        usaurio2      char(10)    CONST;
        WWCERT        char(1)     CONST;
        WWMAUT        char(1)     CONST;
    end-pr;
...
//==============================================================================
    BEGSR inicializo_variables;
     ...
     EXEC SQL select AASFEN into :fecha_cred from sgsysv;
  ...
  //Seccion ODE
   ...
   //Seccion CONTACTOS
    ...
    //Seccion Direccion
      ...
      nro_puerta    = %DEC(%TRIM(CAMPO8):6:0);
      //formateo
      barrio        = %TRIM(CAMPO9);
      ...
      codigo_postal = %DEC(%TRIM(CAMPO10):5:0);
      ENDSR;
...
//=========================================================================
   BEGSR realizar_deb_cred;
      ...
      callSBACMOVI(sucursal_cuenta: //Sucural cuenta cliente
                   nro_cuenta:      //Cuenta Cliente 'AC'
                   fecha_deb:       //Fecha alta
                   hora:            //Hora alta
                   moneda_deb:      //Moneda
     ...
     callSBCCMOVI (suc_cta_cred:     //Sucursal Cuenta Banco
                   nro_cta_cred:     //Nro cuenta del Banco 'CC'
                   fecha_cred:       //Fecha Alta
                   hora:             //Hora alta
                   1:                //Moneda
                   *ZEROS:           //Residencia
                   ...
                   *ZEROS:           //Sucursal Alta
                   ...
                   nro_rrn:          //Nro de cheque
                   fecha_cred:       //Fecha Asiento
                   *ZEROS:           //Asiento contrasentado
                   *ZEROS:           //Fecha asiento contrasenadd
                   1:                //Sector cuentas (Confirmar)
                   *ZEROS:           //Residencia pais
                   'S':              //Generacion asiento contrasentado
                   *BLANKS:          //Usuario
                   *BLANKS:          //Usuario
                   *BLANKS:          //WWCERT
                   *BLANKS);         //WWMAUT
