...
|+---C                   ENDDO
     ...
     C     MARVUE        BEGSR
     C/EXEC SQL
     C+ UPDATE PRHAVU SET HORA = '999' WHERE DOCU = :DOCU AND AREA= :AREA
     C+ AND NOMB= :NOMB AND SEXO =:SEXO
     C/END-EXEC
     C                   ENDSR
     C*-------------------------------------------------------------------------
     C* PRCCRE2: Proceso Créditos solo iguales
     C*-------------------------------------------------------------------------
     C     PRCCRE2       BEGSR
     C                   Z-ADD     WW$BAN        WW$IMO
|||| C                   EXSR      SAVIMO
...
|    C/EXEC SQL
     ...
     C+  SEXO,
     C+ SUM(COD570+COD571+ COD754+COD755), SUM(0)
     C+ FROM PRHAVU
     C+ WHERE HORA <> '999'
     C+ GROUP BY
