...
//Consulta para saber si exite archivos de empleados
        exec sql DECLARE C2 CURSOR FOR
         ...
         WHERE (CAST (FILENAME as char(255)) like '%APSUE%' OR
                CAST (FILENAME as char(255)) like '%APLIT%')
...
//==============================================================================
    ...
    //Ejecuto script para insertar las padron.
    if %subst(nombre_archivo:1:5) = 'APSUE';
    comando_ssh =  'ssh -p 998 ' + %TRIM(url)                               +
                ' "php '+%trim(carpeta)+'cubix/bin/agregaEmpleados.php" '   +
                  '> /home/tmp/prueba.txt 2>&1';
    else;
    comando_ssh =  'ssh -p 998 ' + %TRIM(url)                                 +
                ' "php '+%trim(carpeta)+'cubix/bin/agregaBeneficiarios.php" ' +
                  '> /home/tmp/prueba.txt 2>&1';
    endif;
